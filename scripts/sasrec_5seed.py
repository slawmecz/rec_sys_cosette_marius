#!/usr/bin/env python3
"""
Train + evaluate SASRec++ for multiple seeds on an Amazon 2014 category.
...
CATEGORY_DEFAULTS = {
    "Beauty": {"slug": "beauty", "vocab_size": 12103},
    "Sports_and_Outdoors": {"slug": "sports", "vocab_size": 18359},
}
...
def find_run_for_seed(
    ...
    best_mtime = 0.0
    for run_dir in models_dir.glob("SASRec_*"):
...
def write_summary_table(
    *,
    ...
    ind_stamped, ind_latest = _write_table_files(
        ...
        mode=mode,
        stem=f"table5_{slug}_5seed_individual",
...
def process_seed(
    *,
    ...
    if not run_dir:
        ...
        print(f"[seed {seed}] could not locate SASRec run directory", flush=True)
        return False
...
def resolve_vocab_size(category: str, explicit: int | None) -> int:
    if explicit is not None:
        return explicit
    if category in CATEGORY_DEFAULTS:
        return CATEGORY_DEFAULTS[category]["vocab_size"]
    env_val = os.environ.get("SASREC_VOCAB_SIZE")
    if env_val:
        return int(env_val)
    if category == "Beauty" and os.environ.get("BEAUTY_VOCAB_SIZE"):
        return int(os.environ["BEAUTY_VOCAB_SIZE"])
    raise ValueError(
        f"Set --vocab-size or SASREC_VOCAB_SIZE for category {category}"
    )


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="SASRec++ multi-seed train+test")
    parser.add_argument(
        "--mode",
        choices=["smoke", "full"],
        required=True,
        help="smoke: ~2 min pipeline check; full: paper-aligned 5-seed run",
    )
    parser.add_argument(
        "--category",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="SASRec vocab size (= #items + 2 special tokens)",
    )
    parser.add_argument(
        "--project-root",
    ...
    vocab_size = resolve_vocab_size(category, args.vocab_size)
    metrics = metric_keys(category)
    paper_targets = PAPER_SASREC_PP.get(category, {})

    project_root = Path(args.project_root)
    output_root = Path(args.output_root)
    results_dir = output_root / "results"
    scores_file = results_dir / f"sasrec_{slug}_5seed_{args.mode}_scores.jsonl"
