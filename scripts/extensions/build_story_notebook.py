...
md(
    ...
    "popular items. On Sports this reverses: coverage "
...
md(
    ...
    "\n"
    "A natural extension is the scale axis. Larger datasets change several "
    "variables at once: catalog size, supervision per item, model capacity, "
    "and code-space load. Teammates are now running larger-dataset "
    "reproductions; the important next step is to attach the same "
    "reachability, unreachable-demand, prefix-family survival, and "
    "exact-versus-beam diagnostics to those runs. Do not infer the scale "
    "answer from accuracy alone.\n"
    "\n"
    "The current post-presentation handoff is `PROJECT_STATUS.md`. The "
    "reproduction report is `REPLICATION_REPORT.md`, and extension pilot "
    "outcomes are indexed in `reports/extensions/PILOTS.md`."
)
