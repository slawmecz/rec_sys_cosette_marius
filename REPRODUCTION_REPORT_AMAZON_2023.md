...
- Reference: [`2t2c/cosette_and_marius`](https://github.com/2t2c/cosette_and_marius)
  (`REPRODUCIBILITY.md`), single seed, 2× A100 80 GiB, verified at commit `9a9d8e4`
...
| Reference | not run (open TODO in their `REPRODUCIBILITY.md`) | | | |
...
| B | COSETTE ×5 seeds | `sbatch jobs/arts2023/B_cosette.sbatch` | gpu_h100 ×1 (array 42,44,46,48,50) | per-seed tokenizer |
| C | Collisions + MARIUS ×5 | `sbatch jobs/arts2023/C_marius.sbatch` | gpu_h100 ×2 (array 42,44,46,48,50) | needs B per seed |
| D | SASRec++ ×5 seeds | `sbatch jobs/arts2023/D_sasrec.sbatch` | gpu_h100 ×2 (array 42,44,46,48,50) | independent of B/C |
...
- **Steps per epoch under `num_workers=2`.** With `total_len = max_steps × batch` split
...
|----------|-------|--------------|-----|
| `optim.batch_size` (COSETTE) | `1024` | 256 | reference intent + paper; bs=256 → ~4× steps |
