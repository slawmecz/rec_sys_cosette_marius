# Reproducibility & Replication Report — COSETTE & MARIUS
...
(Lepage, Mary, Picard — CRITEO AI Lab & ENPC; [arXiv:2508.14910](https://arxiv.org/abs/2508.14910)),
for the Recommender Systems master's course.

**Bottom line.** We **run the full mandatory scope** (Beauty + Sports; SASRec++ and
the generative MARIUS-COSETTE; Recall + NDCG; 5 seeds) and report it with mean ± std.
The paper's **absolute numbers are not fully matched** — every test metric lands
~7–28% below — so this is an *honest, well-characterized* reproduction rather than an
...
paper-faithful re-run that **recovers ~55% of the Beauty SASRec++ gap** and confirms
...
validation→test generalization gap) is the one hypothesis we could *not* yet rule
out, and we label it as such.

---

## 1. Acknowledgments & code provenance
...
1. **Original authors** — Simon Lepage, Jérémie Mary, David Picard. Official
   ...
   and unmodified** (verified by `diff`; see §5).
...
2. **Master TA (Udit Thakur)** — fork <https://github.com/2t2c/cosette_and_marius>,
   ...
   **mandatory Amazon-2014 datasets** (a data path the original repo lacked),
   ...
   now superseded — see §5). (TIGER/LETTER references were offered by TA Aswin
   Krishna Mahadevan; not used here.)

3. **Our team** — Stanisław Wasilewski, Sławek, Maks.
   - `slawek`: the full **5-seed** reproduction on **Beauty + Sports** for
     **SASRec++ and MARIUS (COSETTE)** with **Recall + NDCG**.
...
## 2. Mandatory task → delivered

| Requirement (project brief) | Delivered |
|---|---|
| Datasets: **Beauty, Sports** (Amazon-2014) | ✅ both |
| Baseline: **SASRec++** | ✅ + the generative MARIUS-COSETTE it is compared to |
...
**Pipeline** (Amazon-2014, per category): SNAP 5-core reviews → parquet with
**leave-one-out** split (last item = test, 2nd-last = valid, rest = train) →
Sentence-T5-XL embeddings → COSETTE tokenizer (128-dim, 256×4 codebooks, L=4) →
collision removal → train SASRec++ / MARIUS. Dataset sizes after 5-core match the
...
the **full item corpus** (not sampled negatives); R@k / NDCG@k at k ∈ {5,10};
...
- **SASRec++**: the first reproduction used the repo's **default** (d=128,
  ...
  no normalization** (Figure 11a star + Appendix C.1). See §6.
...
The R@10 figure (6.31 → **6.72**) had hidden that COSETTE *beats* SASRec++ on
...
paper **overrides per dataset**; see §6.)*
...
larger on sparser Sports. We traced the cause (§7) and re-ran SASRec++ faithfully.
...
per-seed scores. Artifacts: `reports/paper_faithful_rerun/` (+ the Snellius CC's
`FINDINGS.md`). **On Sports, d=32 and d=64 are within one std on R@10 (a statistical
tie), so model size is not the lever there — both leave a ~20–21% residual.** On
...
- **Settled (robust): the paper's per-dataset config beats the repo default.**
  ...
  d=32 + no-norm), recovering **~55%** of the gap ((9.06−8.24)/(9.73−8.24)). The
...
- **Directional (weaker): smaller helps on Beauty, ties on Sports.** On Beauty,
  d=32 (9.06) edges d=64 (8.88) by ~0.18 R@10 — about one combined std, a modest but
...
- **A residual remains** — Beauty **−0.67 R@10 (~7%)**, Sports **≈−1.3 (~20–21%,
  both configs within noise)**. The ruled-out causes (undertraining, split, eval,
  batch, COSETTE quality) are positively excluded (§7); the residual *cause* is a
  hypothesis (validation→test generalization), not yet positively demonstrated.
...
- **Undertraining** — an automated hypothesis claimed ~3,200 of 80,000 steps. This
  is **false**: the metric CSVs log W&B's `_step` (a throttled logging counter),
  not optimizer steps. Wall-clock (~34–42 min/seed SASRec on H100/A100), the 7–8
  validation events at the configured `val_check_interval=10000`, and SASRec
  validation **peaking then declining** all confirm full ~80k-step, converged
  training.
...
paper's per-dataset Beauty config. Fixing it (d=128→32, L2→none) recovered ~55% of
the Beauty gap; the small-model *direction* reproduces on Beauty (d=32 edges d=64 by
~1 std) and ties on Sports (§6.2). The d=32-vs-128 question was a Figure-11
...
**Remaining residual (~7% Beauty, ~20–21% Sports):** unlike the factors above, the
residual *cause* is **not positively demonstrated** — it is the one hypothesis we
could not rule out. It is **most consistent with** a validation→test generalization
...
only ~7–8 candidates) and small/sparse-data variance that the paper's exact
(unpublished) seeds and per-run tuning may absorb. The direct test (finer
`val_check_interval`) is open work (§9), so we state this as a hypothesis, not a
finding.

**MARIUS note.** MARIUS's config already matches the paper, yet its residual on
Beauty (8.17 vs 10.02, ~18%) is *larger* than faithful-SASRec++'s — so the
...
- The **full mandatory scope is run/covered** (both datasets, the SASRec++ baseline
  + the generative model, Recall + NDCG, 5 seeds) — but the paper's **absolute
  numbers are not matched**: residual ~7% (SASRec++ Beauty), ~20–21% (Sports), and
...
- We **identified and corrected a real configuration discrepancy** (repo default vs
  ...
  controlled 5-seed re-run that recovers ~55% of the Beauty gap; the paper's
  *smaller-is-better* direction reproduces on Beauty (d=32 edges d=64 by ~1 std) and
  ties on Sports.
- The **qualitative claims** we can check: small models help on small data
  (✅ on Beauty; tied on Sports); COSETTE-MARIUS being *competitive-with / above*
...
- The **residual gap is characterized, not hand-waved**: undertraining, split, eval,
  ...
  is *most consistent with* (but not yet proven to be) a validation→test
  generalization gap + checkpoint-selection granularity — stated as a hypothesis.
...
## 9. Honest deviations, limitations & open items

- SASRec++ **head dimension** for small `d` is unspecified by the paper; we keep
  2-head attention (d=32 → d_head=16).
...
- **Sports (18k)** is not in the paper's SASRec++ sizing study; its faithful size
  is interpolated (we bracketed d=32 / d=64 — they tie).
...
- **Open items (next session):** (i) MARIUS faithful re-run + finer-checkpoint
  probe (job 21, needs COSETTE `-col` tokens via jobs 04→05→06); (ii) finer
