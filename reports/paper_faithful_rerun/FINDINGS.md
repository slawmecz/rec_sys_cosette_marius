# Paper-faithful SASRec++ re-run: results and findings
...
1. **Config mismatch was a real, major contributor: confirmed.** Beauty R@10 went
   from 8.24 (repo default d=128 + L2) to 9.06 (paper-faithful d=32 + no-norm).
   That closes about 55% of the gap to the paper's 9.73 ((9.06-8.24)/(9.73-8.24)).
...
- Test the checkpoint-selection / generalization hypothesis on Beauty d=32: finer
  `val_check_interval`, select best-val checkpoint, inspect val-to-test drop per seed.
- The MARIUS finer-checkpoint probe (job 21) is still optional and needs COSETTE
  `-col` tokens (jobs 04 to 05 to 06) which are NOT regenerated on this account yet.
...
- Ran on `gpu_h100` (a100 partition was saturated); about 34-36 min/seed, all 20 seeds
